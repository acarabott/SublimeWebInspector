...
class Protocol(object):
    ...
    def thread_callback(self):
        ...
        self.socket = websocket.WebSocketApp(
            self.url, on_message=self.message_callback, on_open=self.open_callback, on_close=self.close_callback)
    ...
    def message_callback(self, ws, message):
        ...
        if utils.get_setting('ws_debug_mode'):
            print('SWI: <<- ' + json.dumps(parsed, sort_keys=True,
                                           indent=4, separators=(',', ': ')))
        if 'method' in parsed:
            if parsed['method'] in self.notifications:
                ...
                self.to_main_thread(notification.callback,
                                    (data, notification))
        else:
            if parsed['id'] in self.commands:
                ...
                if 'error' in parsed:
                    ...
                    self.to_main_thread(
                        print, ("Error from debuggee: " + parsed['error']['message'], ))
                else:
                    if 'result' in parsed:
