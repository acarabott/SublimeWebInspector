...
assets_path = os.path.abspath(
    "tests" + os.path.sep + "assets").lower() + os.path.sep
...
class SourceMapParserTests(unittest.TestCase):
    ...
    def test_valid_json_invalid_map_does_not_throw(self):
        file_name = Sourcemap.get_sourcemap_file(
            "randomjson.js")  # exists, but not valid map file
        self.assertFalse(file_name)

    def test_version_check(self):
        line_mappings = Sourcemap.SourceMapParser.calculate_line_mappings(
            json.loads(self.map_version_2))
        self.assertFalse(line_mappings)

        line_mappings = Sourcemap.SourceMapParser.calculate_line_mappings(
            json.loads(self.map_version_3))
    ...
    def test_line_mapping_count(self):
        ...
        line_mappings = Sourcemap.SourceMapParser.calculate_line_mappings(
            content)
        self.assertEqual(len(line_mappings), len(
            re.split('[,;]', content["mappings"])))
...
class ParsedSourceMapTests(unittest.TestCase):
    ...
    def test_valid_json_invalid_map_does_not_throw(self):
        parsed_map = Sourcemap.ParsedSourceMap(
            assets_path + "randomjson.js.map")  # exists, but not valid map file
        self.assertFalse(parsed_map.is_valid())
    ...
    def test_valid_map_authored_list(self):
        ...
        self.assertEqual(parsed_map.get_authored_sources_path(), [
                         assets_path + "app.ts"])


class LineMappingsTests(unittest.TestCase):
    def test_empty_mappings(self):
        self.assertFalse(Sourcemap.LineMapping.binary_search(
            [], 0, 0, Sourcemap.LineMapping.compare_source_mappings))
        self.assertFalse(Sourcemap.LineMapping.binary_search(
            [], 0, 0, Sourcemap.LineMapping.compare_generated_mappings))

    def test_binary_search_source_mappings(self):
        parsed_map = Sourcemap.ParsedSourceMap(assets_path + "app.js.map")
        result = Sourcemap.LineMapping.binary_search(
            parsed_map.line_mappings, 10, 16, Sourcemap.LineMapping.compare_source_mappings)
    ...
    def test_binary_search_generated_mappings(self):
        parsed_map = Sourcemap.ParsedSourceMap(assets_path + "app.js.map")
        result = Sourcemap.LineMapping.binary_search(
            parsed_map.line_mappings, 5, 16, Sourcemap.LineMapping.compare_generated_mappings)
    ...
    def test_binary_search_not_existent_source_mappings(self):
        parsed_map = Sourcemap.ParsedSourceMap(assets_path + "app.js.map")
        result = Sourcemap.LineMapping.binary_search(
            parsed_map.line_mappings, 100, 1000, Sourcemap.LineMapping.compare_source_mappings)
    ...
    def test_binary_search_not_existent_generated_mappings(self):
        parsed_map = Sourcemap.ParsedSourceMap(assets_path + "app.js.map")
        result = Sourcemap.LineMapping.binary_search(
            parsed_map.line_mappings, 100, 1000, Sourcemap.LineMapping.compare_generated_mappings)
